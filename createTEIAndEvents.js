...
function saveDataToTracker(cluster_tei, clusterEvents, oneCaseEvent, clusterType, callback) {
    __logger.info("Updated Cluster with UID ---" + cluster_tei)
    getClusterID(function (clusterID) {
        ...
        function doMerging(current, old) {
            ...
            for (var i = 0; i < current.attributes.length; i++) {
                ...
                if (current.attributes[i].attribute == constant.CLUSTER_TEA_CLUSTER_INDEX_DATE) {
                    continue
                }
                ;
                if (current.attributes[i].attribute == constant.CLUSTER_TEA_CLUSTERID) {
                    continue
                }
                ;
